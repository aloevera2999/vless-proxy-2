/**
 * VLESS Proxy Node 2 - Cloudflare Pages Functions
 * 使用新加坡节点 (152ms) 作为代理
 */
...
// 默认代理IP - 使用新加坡节点（152ms延迟）
let proxyIP = '162.159.204.1';
...
const backupProxyIPs = [
    '162.159.204.1',  // 新加坡节点 - 152ms（本节点主用）
    '162.159.201.1',  // 日本节点 - 51ms（备用）
...
export async function onRequest(context) {
    ...
    if (url.pathname === '/' || url.pathname === '/health') {
        return new Response(JSON.stringify({
            status: 'ok',
            service: 'VLESS Proxy Node 2 - Pages Functions',
            version: '2.0',
            uuid: UUID,
            domain: url.hostname,
            proxy_ip: proxyIP + ' (Singapore)',
    ...
    if (url.pathname === '/config') {
        const vlessLink = `vless://${UUID}@${url.hostname}:443?encryption=none&security=tls&sni=${url.hostname}&fp=randomized&type=ws&host=${url.hostname}&path=%2F%3Fed%3D2048#VLESS-Node2-SG`;
